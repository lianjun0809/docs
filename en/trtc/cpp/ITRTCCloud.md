...
| --- | --- |
| getTRTCShareInstance | Create a `TRTCCloud` Instance (Singleton Pattern) |
| destroyTRTCShareInstance | Terminate `TRTCCloud` Instance (Singleton Pattern) |
...
| enableSmallVideoStream | Enable picture-in-picture dual-stream encoding mode |
...
| setBeautyStyle | Set special effects like beauty filter, whitening, and rosy skin |
...
| getScreenCaptureSources | Enumerate shareable screens and windows (this interface supports only desktop systems) |
| selectScreenCaptureTarget | Select the screen or window to share (this interface supports only desktop systems) |
...
| removeAllExcludedShareWindow | Remove All Windows from the Exclusion List of Screen Sharing (This Interface Supports Only Desktop Systems) |
...
| removeAllIncludedShareWindow | Remove All Windows from the Contains List of Screen Sharing (This Interface Supports Only Desktop Systems) |
...
| enableLocalVideoCustomProcess | .1 Enable third-party beauty filter for video |
| setLocalVideoCustomProcessCallback | .2 Set video data callback for third-party beauty filter |
| setLocalVideoRenderCallback | Set Local Video Custom Rendering Callback |
| setRemoteVideoRenderCallback | Sets the rendering callback for remote video self-definition |
| setAudioFrameCallback | Sets the audio data self-definition callback |
...
You can obtain various event notifications from the SDK through ITRTCCloudCallback (e.g., Error Code, Warning Code, Audio and Video Status Parameters, etc.).
...
After calling this interface, you will receive the onEnterRoom(result) callback from ITRTCCloudCallback:
...
The result of the cross-room call request will be notified to you via the ITRTCCloudCallback in the onConnectOtherRoom callback.
...
> -  You can set ITRTCCloudCallback for different instances to get their respective event notifications.
...
The result of calling this interface will be notified to you via the ITRTCCloudCallback in the onUpdateOtherRoomForwardMode callback.
...
| Parameters | Description |
| --- | --- |
| streamType | Specify the type of video stream of the user you want to watch. -  High-definition main stream: TRTCVideoStreamTypeBig. -  Low-quality small screen: TRTCVideoStreamTypeSmall (effective only after the remote user enables Dual-channel encoding through enableSmallVideoStream). -  Substream (commonly used for screen sharing): TRTCVideoStreamTypeSub. |
...
> 2. Only when the specified userid enables Dual-channel encoding through enableSmallVideoStream can you watch the user's small screen.
> 
...
> This feature requires the target user to have already enabled Dual-channel encoding mode in advance through enableSmallVideoStream, otherwise, this call will have no actual effect.
> 
...
> You can also set the parameter to a specific speaker device name to capture sound from that speaker (get the speaker devices of type TXMediaDeviceTypeSpeaker from the TXDeviceManager's getDevicesList interface).
> 
...
> 4. If another user in the room is already sharing the screen using an auxiliary road, calling this interface will receive an onError(ERR_SERVER_CENTER_ANOTHER_USER_PUSH_SUB_VIDEO) callback from ITRTCCloudCallback.
> 
...
> 4. iOS and Mac platforms currently support video frames in TRTCVideoPixelFormat_I420 or TRTCVideoPixelFormat_BGRA32 format.
> 

> 5. Windows and Android platforms currently support video frames in TRTCVideoPixelFormat_I420 format only.
> 
...
Once enabled, you can use ITRTCVideoFrameCallback to obtain image frames of specified pixel format and video data structure type.
| Parameters | Description |
| --- | --- |
| bufferType | Specifying video data structure type. |
| enable | Enable third-party video beautification, disabled by default. |
...
| Parameters | Description |
| --- | --- |
| callback | Custom beauty filter callback. See ITRTCVideoFrameCallback. |
...
-  iOS, Mac, and Windows platforms currently support video frames in TRTCVideoPixelFormat_I420 or TRTCVideoPixelFormat_BGRA32 pixel formats.

-  Android platform currently supports video frames in TRTCVideoPixelFormat_I420, TRTCVideoPixelFormat_RGBA32, or TRTCVideoPixelFormat_Texture_2D pixel formats.

| Parameters | Description |
| --- | --- |
| bufferType | Specifying video data structure type |
| callback | Set Definition Rendering Callback |
...
| Parameters | Description |
| --- | --- |
| bufferType | Specifying video data structure type, currently only supports TRTCVideoBufferType_Buffer |
| callback | Set Definition Rendering Callback |
| pixelFormat | Specify the pixel format for callback |
