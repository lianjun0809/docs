...
| onAudioRouteChanged | Current audio routing has changed (applicable only to mobile devices) |
...
| onScreenCaptureStopped | Event callback for stopping screen sharing |
...
Other users in the room can receive data through the `TRTCAudioFrame.extraData` field callback in `onRemoteUserAudioFrame` of TRTCAudioFrameListener.
| Parameters | Description |
| --- | --- |
| frame | Audio data frame in PCM format. |
