...
**参数**:
- `view`: 播放器渲染 View，iOS为UIView，macOS为NSView
...
```objc
// 设置视频画面旋转0度
[player setRenderRotation:V2TXLiveRotation0];
// 设置视频画面旋转90度
[player setRenderRotation:V2TXLiveRotation90];
// 设置视频画面旋转180度
[player setRenderRotation:V2TXLiveRotation180];
// 设置视频画面旋转270度
[player setRenderRotation:V2TXLiveRotation270];
```
...
```objc
// 图像铺满屏幕
[player setRenderFillMode:V2TXLiveFillModeFill];
// 图像适应屏幕
[player setRenderFillMode:V2TXLiveFillModeFit];
// 图像拉伸铺满
[player setRenderFillMode:V2TXLiveFillModeScaleFill];
```
...
```objc
// 开启镜像模式
[player setRenderMirrorMode:YES];
// 关闭镜像模式
[player setRenderMirrorMode:NO];
```
...
- 支持的类型组合：
  V2TXLivePixelFormatTexture2D, V2TXLiveBufferTypeTexture（推荐）
...
#### enablePictureInPicture() - 开启/关闭画中画功能（iOS特有）
```objc
- (V2TXLiveCode)enablePictureInPicture:(BOOL)enable;
```

**参数**:
- `enable`: YES开启，NO关闭【默认值】：NO
...
**参数**:
- `params`: 请参考 `V2TXLiveDef.h` 中关于 `V2TXLiveLocalRecordingParams` 的介绍
...
- (void)onLocalRecording:(long)durationMs storagePath:(NSString *)storagePath {
    NSLog(@"录制中..., durationMs: %ld", durationMs);
}
...
3. **线程安全**: 所有接口调用需在主线程执行
4. **内存管理**: 及时释放播放器资源，避免内存泄漏
5. **平台差异**: 画中画功能仅iOS平台支持，macOS不支持

## 相关接口

- `V2TXLivePlayerObserver`: 播放器回调接口
